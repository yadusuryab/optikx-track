...
function parseExtractedData(rawText: string): ExtractedData {
  ...
  // ✅ Indian phone number: exactly 10 digits, optionally prefixed with +91 or 0
  // ❌ Excludes tracking numbers like R5000034414, R68400994, PIN codes (6 digits), etc.
  const indianPhonePattern = /(?<![A-Z0-9])(\+91[\s-]?|0)?[6-9]\d{9}(?!\d)/g;

  // ✅ Tracking number pattern - explicitly exclude these
  const trackingNumberPattern = /\b[A-Z]{1,3}\d{7,}\b/g;

  // Enhanced patterns for different data types
  const phoneMatches: string[] = [];
  for (const line of lines) {
    // Skip lines that look like tracking number lines (barcode sticker lines)
    if (trackingNumberPattern.test(line)) {
      console.log('Skipping tracking number line:', line);
      trackingNumberPattern.lastIndex = 0; // reset regex state
      continue;
    }

    // Also skip lines with labels like "PIN", "Pincode", "R5..."
  ...
  if (phoneMatches.length > 0) {
    // Take the last phone number found (phones tend to be at the bottom of labels)
